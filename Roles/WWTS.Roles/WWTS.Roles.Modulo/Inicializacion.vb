...
Public Class Inicializacion
  ...
  Public Function CargarListaGrupoOpciones(ByVal _Sistema As Infoware.Consola.Base.Sistema) As System.Collections.Generic.List(Of Infoware.Consola.Base.GrupoOpcion) Implements Infoware.Consola.Base.IAplicacion.CargarListaGrupoOpciones
    ...
    If _Sistema.Usuario.Restricciones.porModulo(Enumerados.EnumModulos.Reclutamiento) Then
      _grupos.AddRange(New Loader().CargarModuloReclutamiento(_Sistema))
    End If

    If _Sistema.Usuario.Restricciones.porModulo(Enumerados.EnumModulos.Roles) Then
      _grupos.AddRange(New Loader().CargarModuloRoles(_Sistema))
    End If

    'If _Sistema.Usuario.Restricciones.porModulo(Enumerados.EnumModulos.Siembra) Then
    '  _grupos.AddRange(New WWTS.Siembra.Modulo.Loader().CargarModuloSiembra(_Sistema))
    'End If
  ...
  Public Async Function ComprobarActualizaciones(ensamblado As String) As Task(Of Boolean) Implements IAplicacion.ComprobarActualizaciones
    ...
    Using mgr As FUpdateManager = Await GitHubUpdateManager.GetUpdateManager("vicosanz", "RolesInstaller", ensamblado)
