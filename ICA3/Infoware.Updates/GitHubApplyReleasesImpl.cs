...
{
    public class GitHubApplyReleasesImpl : IEnableLogger
    {
        private string rootAppDirectory;

        public GitHubApplyReleasesImpl(string rootAppDirectory)
        {
        ...
        public async Task<string> ApplyReleases(UpdateInfo updateInfo, bool silentInstall, bool attemptingFullInstall, Action<int> progress = null)
        {
            ...
            var ret = await this.ErrorIfThrows(() => installPackageToAppDir(updateInfo, release, x => progress(FUtilities.CalculateProgress(x, 40, 80))),
            ...
            var appDir = new DirectoryInfo(FUtilities.AppDirForRelease(rootAppDirectory, updateInfo.FutureReleaseEntry));
        ...
        public async Task FullUninstall()
        {
            ...
            if (currentRelease.Exists)
            {
                ...
                try
                {
                    var squirrelAwareApps = FSquirrelAwareExecutableDetector.GetAllSquirrelAwareApps(currentRelease.FullName);
                    ...
                    if (squirrelAwareApps.Count > 0)
                    {
                        await squirrelAwareApps.ForEachAsync(async exe =>
                        {
                            using (var cts = new CancellationTokenSource())
                            {
                                ...
                                try
                                {
                                    await FUtilities.InvokeProcessAsync(exe, String.Format("--squirrel-uninstall {0}", version), cts.Token);
                                }
                                catch (Exception ex)
                                {
            ...
            await this.ErrorIfThrows(() => FUtilities.DeleteDirectoryOrJustGiveUp(rootAppDirectory),
        ...
        public Dictionary<ShortcutLocation, ShellLink> GetShortcutsForExecutable(string exeName, ShortcutLocation locations, string programArguments)
        {
            ...
            var releases = FUtilities.LoadLocalReleases(FUtilities.LocalReleaseFileForAppDir(rootAppDirectory));
            var thisRelease = FUtilities.FindCurrentVersion(releases);

            var zf = new ZipPackage(Path.Combine(
                FUtilities.PackageDirectoryForAppDir(rootAppDirectory),
                thisRelease.Filename));

            var exePath = Path.Combine(FUtilities.AppDirForRelease(rootAppDirectory, thisRelease), exeName);
            ...
            foreach (var f in (ShortcutLocation[])Enum.GetValues(typeof(ShortcutLocation)))
            {
                ...
                var toastActivatorCLSDID = FUtilities.CreateGuidFromHash(appUserModelId).ToString();
        ...
        public void CreateShortcutsForExecutable(string exeName, ShortcutLocation locations, bool updateOnly, string programArguments, string icon)
        {
            ...
            foreach (var f in (ShortcutLocation[])Enum.GetValues(typeof(ShortcutLocation)))
            {
                ...
                this.ErrorIfThrows(() => FUtilities.Retry(() =>
                {
                    ...
                    var appUserModelId = String.Format("com.squirrel.{0}.{1}", zf.Id.Replace(" ", ""), exeName.Replace(".exe", "").Replace(" ", ""));
                    var toastActivatorCLSID = FUtilities.CreateGuidFromHash(appUserModelId).ToString();
        ...
        public void RemoveShortcutsForExecutable(string exeName, ShortcutLocation locations)
        {
            ...
            var fileVerInfo = FileVersionInfo.GetVersionInfo(
                Path.Combine(FUtilities.AppDirForRelease(rootAppDirectory, thisRelease), exeName));
        ...
        Task<string> installPackageToAppDir(UpdateInfo updateInfo, ReleaseEntry release, Action<int> progressCallback)
        {
            return Task.Run(async () =>
            {
                ...
                if (target.Exists)
                {
                    ...
                    await FUtilities.DeleteDirectory(target.FullName);
                }
        ...
        async Task<ReleaseEntry> createFullPackagesFromDeltas(IEnumerable<ReleaseEntry> releasesToApply, ReleaseEntry currentVersion, FApplyReleasesProgress progress)
        {
            Contract.Requires(releasesToApply != null);

            progress = progress ?? new FApplyReleasesProgress(releasesToApply.Count(), x => { });
        ...
        async Task invokePostInstall(SemanticVersion currentVersion, bool isInitialInstall, bool firstRunOnly, bool silentInstall)
        {
            ...
            var squirrelApps = FSquirrelAwareExecutableDetector.GetAllSquirrelAwareApps(targetDir.FullName);
            ...
            if (!firstRunOnly) await squirrelApps.ForEachAsync(async exe =>
            {
                using (var cts = new CancellationTokenSource())
                {
                    ...
                    try
                    {
                        await FUtilities.InvokeProcessAsync(exe, args, cts.Token);
                    }
                    catch (Exception ex)
                    {
        ...
        void fixPinnedExecutables(SemanticVersion newCurrentVersion, bool removeAll = false)
        {
            ...
            foreach (var shortcut in shellLinks)
            {
                try
                {
                    ...
                    if (removeAll)
                    {
                        FUtilities.DeleteFileHarder(shortcut.ShortCutFile);
                    }
                    else
                    {
        ...
        void updateLink(ShellLink shortcut, string newAppPath)
        {
            ...
            this.ErrorIfThrows(() => FUtilities.Retry(() => shortcut.Save(), 2), "Couldn't write shortcut " + shortcut.ShortCutFile);
        ...
        async Task cleanDeadVersions(SemanticVersion currentVersion, SemanticVersion newVersion, bool forceUninstall = false)
        {
            ...
            await toCleanup.ForEachAsync(async x =>
            {
                try
                {
                    ...
                    {
                        await FUtilities.DeleteDirectoryOrJustGiveUp(x.FullName);
                    }
            ...
            var releasesFile = FUtilities.LocalReleaseFileForAppDir(rootAppDirectory);
            var entries = ReleaseEntry.ParseReleaseFile(File.ReadAllText(releasesFile, Encoding.UTF8));
            var pkgDir = FUtilities.PackageDirectoryForAppDir(rootAppDirectory);
        ...
        {
            return await Task.Run(() => ReleaseEntry.BuildReleasesFile(FUtilities.PackageDirectoryForAppDir(rootAppDirectory)));
        }
