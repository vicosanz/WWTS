...
{
    internal class GitHubInstallHelperImpl : IEnableLogger
    {
        private string applicationName;
        private string rootAppDirectory;

        public GitHubInstallHelperImpl(string applicationName, string rootAppDirectory)
        {
        ...
        public async Task<RegistryKey> CreateUninstallerRegistryEntry(string uninstallCmd, string quietSwitch)
        {
            ...
            if (zp.IconUrl != null && !File.Exists(targetIco))
            {
                try
                {
                    using (var wc = FUtilities.CreateWebClient())
                    {
