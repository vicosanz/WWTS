...
Public Class ItemxBodega
  ...
  Public Overridable ReadOnly Property ItemSerieString() As String
    Get
      If mItem Is Nothing Then
        Return String.Empty
      Else
        If mItem.Item_esRegistroSerie Then
          Return "Si"
        Else
          Return "No"
        End If
...
Public Class ItemxBodegaList
  ...
  '-para bodega de dañados principalmete
  Public Shared Function ObtenerListaItemxBodega(ByVal _Bodega As Bodega, ByVal _Item As Item, Optional ByVal _Sucursal As Sucursal = Nothing) As ItemxBodegaList
    Dim oResult As ItemxBodegaList = New ItemxBodegaList
    Dim bReturn As Boolean
    Dim ds As DataTable = Nothing
    ...
    If bReturn AndAlso Not ds Is Nothing AndAlso ds.Rows.Count > 0 Then
      For Each _dr As DataRow In ds.Rows
        Dim _fila As New ItemxBodega()
        _fila.MapearDataRowaObjeto(_dr)
        _fila.Item = New Item(_Sucursal.OperadorDatos, _fila.Item_Codigo)
        oResult.Add(_fila)
      Next
    End If
    Return oResult
  ...
  '-para bodega de dañados principalmete
  Public Shared Function ObtenerListaItemxBodegaComponentes(ByVal _Bodega As Bodega, ByVal _Item As Item, Optional ByVal _Sucursal As Sucursal = Nothing) As ItemxBodegaList
    Dim oResult As ItemxBodegaList = New ItemxBodegaList
    Dim bReturn As Boolean
    Dim ds As DataTable = Nothing
    With _Sucursal.OperadorDatos
      .AgregarParametro("@Accion", "FI")
      .AgregarParametro("@Bodega_Codigo", _Bodega.Bodega_Codigo)
      If _Sucursal IsNot Nothing Then
        .AgregarParametro("@Empres_Codigo", _Sucursal.Empres_Codigo)
        .AgregarParametro("@Sucurs_Bodega", _Sucursal.Sucurs_Codigo)
