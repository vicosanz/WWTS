...
Public Class CtlBuscaCliente
  ...
  Sub Llenar_Datos()
    ...
    mClientes = ClienteList.ObtenerLista(mSucursal.OperadorDatos)
  ...
  Private Sub abrir_consulta(Optional ByVal filtro As String = "")
    Dim f As New FrmListaClientes(Sistema, Enumerados.EnumOpciones.ListadoClientes, True) With {
      .Sucursal = mSucursal,
      .Cliente = Cliente,
      .Filtro = filtro
    }
  ...
  Sub Abrir_Mantenimiento()
    Dim f As New FrmMantenimientoCliente(Sistema, Enumerados.EnumOpciones.ListadoClientes, mSucursal)
    f.Cliente = New Cliente(Sistema.OperadorDatos, True)
    f.ShowDialog()

    If f.Cliente IsNot Nothing AndAlso Not f.Cliente.EsNuevo Then
      Llenar_Datos()
      ComboBox1.SelectedValue = f.Cliente.Entida_Codigo
    End If
