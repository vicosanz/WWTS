...
Public Class CtlTarjCuracion
  ...
  Private Sub TabPage2_Click(sender As Object, e As EventArgs) Handles TabControl1.SelectedIndexChanged
    Try
      If Not TabControl1.SelectedTab Is TabPage2 Then
        Exit Sub
      End If
      Dim info As ReportClass = New crpCuracionTarjeta

      Dim ds As New dsCuracionTarjeta
      ds = RetornarCuracionTarjeta(mOp, mCuracion)
      ...
      Else
        ...
        Me.crpTarjeta.Zoom(1)
        Me.crpTarjeta.ReportSource = info
        Me.crpTarjeta.DisplayToolbar = True
        Me.crpTarjeta.ShowRefreshButton = False
      End If
      Auditoria.Registrar_Auditoria(mRestriccion, Auditoria.enumTipoAccion.Impresion, String.Format("{0} {1}", "Imprimió tarjeta curación", mCuracion.Cur_numero))
  ...
  Public Function RetornarCuracionTarjeta(ByVal _OperadorDatos As OperadorDatos, ByVal _curacion As Curacion) As dsCuracionTarjeta
    Dim bReturn As Boolean = True
    Dim dsResult As New dsCuracionTarjeta
    With _OperadorDatos
      .AgregarParametro("@accion", "p")
      .AgregarParametro("@cur_numero", _curacion.Cur_numero)
      .Procedimiento = "proc_Curacion"
      ...
      Try
        dsResult = New dsCuracionTarjeta
        DataAdapter.Fill(dsResult, "vw_curaciontarjeta")
      Catch ex As Exception
        bReturn = False
      End Try
      .LimpiarParametros()
