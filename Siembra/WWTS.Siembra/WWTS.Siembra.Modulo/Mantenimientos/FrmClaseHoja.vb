
Imports WWTS.Siembra.Reglas
Imports WWTS.General.Modulo

Public Class FrmClaseHoja
  Implements IMantenimiento

  Private mClaseHoja As ClaseHoja
  Public Property ClaseHoja() As ClaseHoja
    Get
      Return mClaseHoja
    End Get
    Set(ByVal Value As ClaseHoja)
      If Value Is Nothing Then
        Throw New NullReferenceException("Debe instanciar un ClaseHoja")
      End If
      mClaseHoja = Value
      Refrescar_forma()
    End Set
  End Property

  Private mDireccion As IMantenimiento.Accion
  Public Property Direccion() As IMantenimiento.Accion Implements IMantenimiento.Direccion
    Get
      Return mDireccion
    End Get
    Set(ByVal value As IMantenimiento.Accion)
      mDireccion = value
      Refrescar_forma()
    End Set
  End Property

  Sub Refrescar_forma() Implements IMantenimiento.Refrescar_forma
    If Not ClaseHoja Is Nothing Then
      Me.txtcodigo.Text = ClaseHoja.Cla_codigo
      Me.txtCodigoAux.Text = ClaseHoja.Cla_codigoAux
      Me.txtnombre.Text = ClaseHoja.Cla_nombre
      Me.txtprecio.Text = ClaseHoja.Cla_precio.ToString()
    End If
    Me.txtcodigo.Enabled = Not mDireccion = IMantenimiento.Accion.Modificacion
    Me.txtCodigoAux.Enabled = Not Direccion = IMantenimiento.Accion.Eliminacion
    Me.txtnombre.Enabled = Not Direccion = IMantenimiento.Accion.Eliminacion
    Me.txtprecio.Enabled = Not Direccion = IMantenimiento.Accion.Eliminacion
  End Sub

  Private Sub FrmConexion_AntesAceptar(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles Me.AntesAceptar
    Try
      Select Case mDireccion
        Case IMantenimiento.Accion.Ingreso, IMantenimiento.Accion.Modificacion
          ClaseHoja.Cla_codigo = Me.txtcodigo.Text
          ClaseHoja.Cla_nombre = Me.txtnombre.Text
          ClaseHoja.Cla_codigoAux = Me.txtCodigoAux.Text
          ClaseHoja.Cla_precio = Me.txtprecio.Text
          If Not ClaseHoja.Guardar() Then
            Throw New Exception(ClaseHoja.OperadorDatos.MsgError)
          End If
        Case IMantenimiento.Accion.Eliminacion
          If Not ClaseHoja.Eliminar() Then
            Throw New Exception(ClaseHoja.OperadorDatos.MsgError)
          End If
      End Select
    Catch ex As Exception
      MsgBox(ex.Message, MsgBoxStyle.Critical, "Error")
      e.Cancel = True
    End Try
  End Sub
End Class
